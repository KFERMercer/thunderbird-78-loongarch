...
add_task(async function testAttachProvider() {
  ...
  let listItemImage = listItem.querySelector("image");
  ...
  let eventBox = await getEventBox("calendar-month-day-box-item");
  Assert.equal(eventBox.item.getAttachments()[0].uri.spec, fileUrl, "event has attachment URL");
  ...
  // Clean up.
  eventBox = await getEventBox("calendar-month-day-box-item");
  eventBox.focus();
  EventUtils.synthesizeKey("VK_DELETE", {});
  await CalendarTestUtils.closeCalendarTab(window);
});
