...
add_task(async () => {
  ...
  Assert.equal(childCards[0].UID, "a-new-card");
  ...
  Assert.ok(
    modifiedCard.getProperty("_vCard", "").includes("UID:drac-wen-a\r\n")
  );
  Assert.ok(
    modifiedCard
      .getProperty("_vCard", "")
      .includes("X-MODIFIED-BY-SERVER:1\r\n")
  );
