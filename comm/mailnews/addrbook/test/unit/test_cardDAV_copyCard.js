...
add_task(async () => {
  ...
  let childCards = [...cardDAVDirectory.childCards];
  Assert.equal(childCards.length, 1);

  initialCard = childCards[0];
...
add_task(async function copyCardToLocal() {
  ...
  let childCards = [...localDirectory.childCards];
  Assert.equal(childCards.length, 1);

  localCard = childCards[0];
