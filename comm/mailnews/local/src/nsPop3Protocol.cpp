...
static int net_pop3_remove_messages_marked_delete(PLHashEntry* he, int msgindex,
                                                  void* arg) {
  Pop3UidlEntry* uidlEntry = (Pop3UidlEntry*)he->value;
  return (uidlEntry->status == DELETE_CHAR) ? HT_ENUMERATE_REMOVE
                                            : HT_ENUMERATE_NEXT;
}
...
static void put_hash(PLHashTable* table, const char* key, char value,
...
static int net_pop3_copy_hash_entries(PLHashEntry* he, int msgindex,
                                      void* arg) {
  Pop3UidlEntry* uidlEntry = (Pop3UidlEntry*)he->value;
  put_hash((PLHashTable*)arg, uidlEntry->uidl, uidlEntry->status,
           uidlEntry->dateReceived);
  return HT_ENUMERATE_NEXT;
}

static void* AllocUidlTable(void* /* pool */, size_t size) {
  return PR_MALLOC(size);
}

static void FreeUidlTable(void* /* pool */, void* item) { PR_Free(item); }

static PLHashEntry* AllocUidlInfo(void* pool, const void* key) {
  return PR_NEWZAP(PLHashEntry);
}

static void FreeUidlInfo(void* /* pool */, PLHashEntry* he, unsigned flag) {
...
static PLHashAllocOps gHashAllocOps = {AllocUidlTable, FreeUidlTable,
...
static Pop3UidlHost* net_pop3_load_state(const char* searchhost,
  ...
  result->hash = PL_NewHashTable(20, PL_HashString, PL_CompareStrings,
  ...
  while (more && NS_SUCCEEDED(rv)) {
    ...
    if (firstChar == '*') {
      ...
      current = nullptr;
      char* lineBuf =
          line.BeginWriting() + 1;  // ok because we know the line isn't empty
      char* host = NS_strtok(" \t\r\n", &lineBuf);
      ...
      if (!current) {
        ...
        if (current) {
          ...
          current->hash =
              PL_NewHashTable(20, PL_HashString, PL_CompareStrings,
                              PL_CompareValues, &gHashAllocOps, nullptr);
      ...
      if (current) {
        for (int32_t pos = line.FindChar('\t'); pos != -1;
             pos = line.FindChar('\t', pos))
          line.Replace(pos, 1, ' ');

        nsTArray<nsCString> lineElems;
        ParseString(line, ' ', lineElems);
        if (lineElems.Length() < 2) continue;
        nsCString* flags = &lineElems[0];
        nsCString* uidl = &lineElems[1];
        uint32_t dateReceived = TimeInSecondsFromPRTime(
            PR_Now());  // if we don't find a date str, assume now.
        if (lineElems.Length() > 2) dateReceived = atoi(lineElems[2].get());
        if (!flags->IsEmpty() && !uidl->IsEmpty()) {
          char flag = flags->CharAt(0);
          if ((flag == KEEP) || (flag == DELETE_CHAR) || (flag == TOO_BIG) ||
              (flag == FETCH_BODY)) {
...
static int hash_empty_mapper(PLHashEntry* he, int msgindex, void* arg) {
  *((bool*)arg) = false;
  return HT_ENUMERATE_STOP;
}

static bool hash_empty(PLHashTable* hash) {
  bool result = true;
  PL_HashTableEnumerateEntries(hash, hash_empty_mapper, (void*)&result);
  return result;
}

static int net_pop3_write_mapper(PLHashEntry* he, int msgindex, void* arg) {
...
void nsPop3Protocol::MarkMsgInHashTable(PLHashTable* hashTable,
  ...
  if (uidlE->uidl) {
    Pop3UidlEntry* uidlEntry =
        (Pop3UidlEntry*)PL_HashTableLookup(hashTable, uidlE->uidl);
    if (uidlEntry) {
...
nsresult nsPop3Protocol::MarkMsgForHost(const char* hostName,
  ...
  if (!hostName || !userName || !mailDirectory) return NS_ERROR_NULL_POINTER;

  Pop3UidlHost* uidlHost =
      net_pop3_load_state(hostName, userName, mailDirectory);
  if (!uidlHost) return NS_ERROR_OUT_OF_MEMORY;
...
nsresult nsPop3Protocol::Initialize(nsIURI* aURL) {
  ...
  if (!m_pop3ConData) return NS_ERROR_OUT_OF_MEMORY;
...
nsresult nsPop3Protocol::InitializeInternal(nsIProxyInfo* aProxyInfo) {
  ...
  if (m_socketType != nsMsgSocketType::plain) {
    ...
    if (mailnewsUrl) mailnewsUrl->GetMsgWindow(getter_AddRefs(msgwin));
    if (!msgwin) GetTopmostMsgWindow(getter_AddRefs(msgwin));
    if (msgwin) {
  ...
  rv = OpenNetworkSocketWithInfo(hostName.get(), port, connectionType,
                                 aProxyInfo, ir);
  if (NS_FAILED(rv) && m_socketType == nsMsgSocketType::trySTARTTLS) {
    m_socketType = nsMsgSocketType::plain;
    rv = OpenNetworkSocketWithInfo(hostName.get(), port, nullptr, aProxyInfo,
                                   ir);
  }
  ...
  switch (m_pop3ConData->next_state) {
    ...
    default:
      ...
      MOZ_ASSERT_UNREACHABLE(
          "Invalid next_state in GetNextPasswordObtainState");
  }
...
NS_IMETHODIMP nsPop3Protocol::OnPromptStartAsync(
...
NS_IMETHODIMP nsPop3Protocol::OnPromptStart(bool* aResult) {
  ...
  *aResult = false;
  ...
  if (!m_lastPasswordSent.IsEmpty()) passwordResult = m_lastPasswordSent;
  ...
  if (TestFlag(POP3_PASSWORD_FAILED)) {
    ...
    if (msgWindow) {
      MOZ_LOG(POP3LOGMODULE, LogLevel::Warning,
              (POP3LOG("POP: ask user what to do (after password failed): new "
                       "password, retry or cancel")));

      int32_t buttonPressed = 0;
      if (NS_SUCCEEDED(MsgPromptLoginFailed(msgWindow, hostName, userName,
                                            accountName, &buttonPressed))) {
        if (buttonPressed == 1)  // Cancel button
        {
          MOZ_LOG(POP3LOGMODULE, LogLevel::Warning,
                  (POP3LOG("cancel button pressed")));
        ...
        {
          MOZ_LOG(POP3LOGMODULE, LogLevel::Warning,
                  (POP3LOG("new password button pressed")));
...
NS_IMETHODIMP nsPop3Protocol::OnPromptAuthAvailable() {
  MOZ_ASSERT_UNREACHABLE(
      "Did not expect to get POP3 protocol queuing up auth "
...
NS_IMETHODIMP nsPop3Protocol::OnTransportStatus(nsITransport* aTransport,
                                                nsresult aStatus,
                                                int64_t aProgress,
                                                int64_t aProgressMax) {
  return nsMsgProtocol::OnTransportStatus(aTransport, aStatus, aProgress,
                                          aProgressMax);
}

// stop binding is a "notification" informing us that the stream associated with
// aURL is going away.
NS_IMETHODIMP nsPop3Protocol::OnStopRequest(nsIRequest* aRequest,
                                            nsresult aStatus) {
  ...
  if (m_socketIsOpen) {
    ...
    if ((m_pop3ConData->next_state_after_response == POP3_NEXT_AUTH_STEP ||
         m_pop3ConData->next_state_after_response ==
             POP3_AUTH_LOGIN_RESPONSE) &&
        m_pop3ConData->next_state != POP3_OBTAIN_PASSWORD_EARLY) {
      MOZ_LOG(POP3LOGMODULE, LogLevel::Debug,
              (POP3LOG("dropped connection before auth error")));
  ...
  if (m_pop3ConData->list_done) CommitState(true);
  if (NS_FAILED(aStatus) && aStatus != NS_BINDING_ABORTED) Abort();
  return rv;
}
...
void nsPop3Protocol::Abort() {
  ...
  if (m_nsIPop3Sink) m_nsIPop3Sink->AbortMailDelivery(this);
  MOZ_LOG(POP3LOGMODULE, LogLevel::Debug,
          (POP3LOG("Clearing running protocol in nsPop3Protocol::Abort()")));
  if (m_pop3Server) m_pop3Server->SetRunningProtocol(nullptr);
}
...
nsresult nsPop3Protocol::LoadUrlInternal(nsIURI* aURL) {
  ...
  m_pop3ConData->only_check_for_new_mail =
      (PL_strcasestr(queryPart.get(), "check") != nullptr);
  m_pop3ConData->verify_logon =
      (PL_strcasestr(queryPart.get(), "verifyLogon") != nullptr);
  ...
  if (pop3Url) pop3Url->GetPop3Sink(getter_AddRefs(m_nsIPop3Sink));
  ...
  if (!m_pop3ConData->verify_logon)
    m_pop3ConData->uidlinfo =
        net_pop3_load_state(hostName.get(), userName.get(), mailDirectory);
  ...
  if (m_pop3ConData->uidlinfo && numDaysToLeaveOnServer > 0) {
    ...
    PL_HashTableEnumerateEntries(m_pop3ConData->uidlinfo->hash,
                                 net_pop3_delete_old_msgs_mapper,
                                 (void*)(uintptr_t)cutOffDay);
  }
...
void nsPop3Protocol::FreeMsgInfo() {
  int i;
...
int32_t nsPop3Protocol::WaitForStartOfConnectionResponse(
    nsIInputStream* aInputStream, uint32_t length) {
  char* line = nullptr;
  uint32_t line_length = 0;
  bool pauseForMoreData = false;
  nsresult rv;
  line = m_lineStreamBuffer->ReadNextLine(aInputStream, line_length,
                                          pauseForMoreData, &rv);
  ...
  if (*line == '+') {
    ...
    if (m_prefAuthMethods & POP3_HAS_AUTH_APOP) {
      if (NS_SUCCEEDED(GetApopTimestamp())) SetCapFlag(POP3_HAS_AUTH_APOP);
    } else
...
int32_t nsPop3Protocol::WaitForResponse(nsIInputStream* inputStream,
                                        uint32_t length) {
  char* line;
  uint32_t ln = 0;
  bool pauseForMoreData = false;
  nsresult rv;
  line =
      m_lineStreamBuffer->ReadNextLine(inputStream, ln, pauseForMoreData, &rv);
  if (NS_FAILED(rv)) return -1;
  ...
  if (*line == '+') {
    ...
    if (TestCapFlag(POP3_HAS_RESP_CODES | POP3_HAS_AUTH_RESP_CODE)) {
      ...
      if (i >= 0) m_commandResponse.Cut(0, i + 2);
    }
  }
...
int32_t nsPop3Protocol::Pop3SendData(const char* dataBuffer,
  ...
  if (!aSuppressLogging)
    ...
    MOZ_LOG(POP3LOGMODULE, LogLevel::Info,
            (POP3LOG("Logging suppressed for this command (it probably "
...
int32_t nsPop3Protocol::SendTLSResponse() {
  ...
  if (m_pop3ConData->command_succeeded) {
    ...
    if (NS_SUCCEEDED(rv)) {
      ...
      uint32_t preservedCapFlags =
          m_pop3ConData->capability_flags & POP3_HAS_AUTH_APOP;
      m_pop3ConData->capability_flags =  // resetting the flags
          POP3_AUTH_MECH_UNDEFINED |
          POP3_HAS_AUTH_USER |  // should be always there
          POP3_GURL_UNDEFINED | POP3_UIDL_UNDEFINED | POP3_TOP_UNDEFINED |
          POP3_XTND_XLST_UNDEFINED | preservedCapFlags;
      m_pop3Server->SetPop3CapabilityFlags(m_pop3ConData->capability_flags);
      return 0;
    }
  }
...
int32_t nsPop3Protocol::NextAuthStep() {
  ...
  if (m_pop3ConData->command_succeeded) {
    ...
    MOZ_LOG(POP3LOGMODULE, LogLevel::Debug,
            (POP3LOG("command did not succeed")));
    ...
    if (m_currentAuthMethod == POP3_HAS_AUTH_USER && !m_password_already_sent) {
      MOZ_LOG(POP3LOGMODULE, LogLevel::Debug,
              (POP3LOG("USER username failed")));
...
int32_t nsPop3Protocol::AuthGSSAPIResponse(bool first) {
  if (!m_pop3ConData->command_succeeded) {
    if (first) m_GSSAPICache.Truncate();
    MarkAuthMethodAsFailed(POP3_HAS_AUTH_GSSAPI);
    m_pop3ConData->next_state = POP3_PROCESS_AUTH;
    m_pop3ConData->pause_for_read = false;
    return 0;
...
int32_t nsPop3Protocol::SendUsername() {
  ...
  if (m_username.IsEmpty()) return Error("pop3UsernameUndefined");
  ...
  if (m_currentAuthMethod == POP3_HAS_AUTH_NTLM)
    ...
    char* base64Str =
        PL_Base64Encode(m_username.get(), m_username.Length(), nullptr);
    cmd = base64Str;
    PR_Free(base64Str);
...
int32_t nsPop3Protocol::SendPassword() {
  ...
  if (m_currentAuthMethod == POP3_HAS_AUTH_NTLM)
    ...
    char* decodedChallenge = PL_Base64Decode(
        m_commandResponse.get(), m_commandResponse.Length(), nullptr);
...
int32_t nsPop3Protocol::SendStat() { return SendStatOrGurl(true); }

int32_t nsPop3Protocol::GetStat() {
  // check stat response
  if (!m_pop3ConData->command_succeeded) return Error("pop3StatFail");
  ...
  if (num) {
    ...
    if (num)
      m_totalFolderSize =
          nsCRT::atoll(num);  // we always initialize m_totalFolderSize to 0
  } else
...
int32_t nsPop3Protocol::SendGurl() {
...
int32_t nsPop3Protocol::GurlResponse() {
  ClearCapFlag(POP3_GURL_UNDEFINED);

  if (m_pop3ConData->command_succeeded) {
    SetCapFlag(POP3_HAS_GURL);
    if (m_nsIPop3Sink) m_nsIPop3Sink->SetMailAccountURL(m_commandResponse);
  } else {
...
int32_t nsPop3Protocol::SendList() {
  // check for server returning number of messages that will cause the
  // calculation of the size of the block for msg_info to overflow a 32 bit int,
  // in turn causing us to allocate a block of memory much smaller than we think
  // we're allocating, and potentially allowing the server to make us overwrite
  // memory outside our heap block.

  if (m_pop3ConData->number_of_messages >
      (int)(0xFFFFF000 / sizeof(Pop3MsgInfo)))
    return MK_OUT_OF_MEMORY;

  m_pop3ConData->msg_info = (Pop3MsgInfo*)PR_CALLOC(
      sizeof(Pop3MsgInfo) * m_pop3ConData->number_of_messages);
  if (!m_pop3ConData->msg_info) return (MK_OUT_OF_MEMORY);
  m_pop3ConData->next_state_after_response = POP3_GET_LIST;
  m_listpos = 0;
  return Pop3SendData("LIST" CRLF);
}

int32_t nsPop3Protocol::GetList(nsIInputStream* inputStream, uint32_t length) {
  /* check list response
   * This will get called multiple times
   * but it's alright since command_succeeded
   * will remain constant
   */
  if (!m_pop3ConData->command_succeeded) return Error("pop3ListFailure");
...
int32_t nsPop3Protocol::HandleNoUidListAvailable() {
  ...
  if (!m_pop3ConData->leave_on_server && !m_pop3ConData->headers_only &&
      m_pop3ConData->size_limit <= 0 && !m_pop3ConData->only_uidl) {
...
int32_t nsPop3Protocol::GetXtndXlstMsgid(nsIInputStream* inputStream,
                                         uint32_t length) {
  /* check list response
   * This will get called multiple times
   * but it's alright since command_succeeded
  ...
  if (token) {
    ...
    if (++m_listpos <= m_pop3ConData->number_of_messages) {
      ...
      const char* uid =
          NS_strtok(" ", &newStr);  // not really a UID but a unique token -km
      if (!uid)
      ...
      if (m_pop3ConData->msg_info[m_listpos - 1].msgnum == msg_num)
        ...
        for (i = 0; i < m_pop3ConData->number_of_messages &&
                    m_pop3ConData->msg_info[i].msgnum != msg_num;
             i++)
          ;
...
int32_t nsPop3Protocol::GetMsg() {
  ...
  if (m_pop3ConData->last_accessed_msg >= m_pop3ConData->number_of_messages) {
    ...
    if (m_pop3ConData->msg_del_started) {
      if (!m_pop3ConData->only_uidl) {
        if (m_pop3ConData->only_check_for_new_mail)
          m_nsIPop3Sink->SetBiffStateAndUpdateFE(
              m_pop3ConData->biffstate, m_pop3ConData->really_new_messages,
              true);
        /* update old style biff */
        else
          m_nsIPop3Sink->SetBiffStateAndUpdateFE(
              nsIMsgFolder::nsMsgBiffState_NewMail,
              m_pop3ConData->really_new_messages, false);
      }
  ...
  if (m_totalDownloadSize < 0) {
    ...
    if (m_pop3ConData->msg_info) {
      ...
      for (int32_t i = 0; i < m_pop3ConData->number_of_messages; i++) {
        ...
        if (m_pop3ConData->msg_info[i].uidl) {
          Pop3UidlEntry* uidlEntry = (Pop3UidlEntry*)PL_HashTableLookup(
              m_pop3ConData->uidlinfo->hash, m_pop3ConData->msg_info[i].uidl);
          if (uidlEntry) {
            c = uidlEntry->status;
        ...
        if ((c == KEEP) &&
            !m_pop3ConData
                 ->leave_on_server) { /* This message has been downloaded but
                                       * kept on server, we no longer want to
                                       * keep it there */
          if (!m_pop3ConData->newuidl) {
            m_pop3ConData->newuidl =
                PL_NewHashTable(20, PL_HashString, PL_CompareStrings,
                                PL_CompareValues, &gHashAllocOps, nullptr);
            if (!m_pop3ConData->newuidl) return MK_OUT_OF_MEMORY;
          }
          c = DELETE_CHAR;
          // Mark message to be deleted in new table
          put_hash(m_pop3ConData->newuidl, m_pop3ConData->msg_info[i].uidl,
                   DELETE_CHAR, popstateTimestamp);
          // and old one too
        ...
        if ((c != KEEP) && (c != DELETE_CHAR) &&
            (c != TOO_BIG)) {  // message left on server
          m_totalDownloadSize += m_pop3ConData->msg_info[i].size;
    ...
    if (m_pop3ConData->only_check_for_new_mail) {
      if (m_totalDownloadSize > 0) {
        ...
        m_nsIPop3Sink->SetBiffStateAndUpdateFE(
            nsIMsgFolder::nsMsgBiffState_NewMail,
            m_pop3ConData->really_new_messages, true);
      }
      m_pop3ConData->next_state = POP3_SEND_QUIT;
      return (0);
    }
    ...
    if (m_totalDownloadSize > 0)  // skip all this if there aren't any messages
    {
      ...
      nsCOMPtr<nsIMsgMailNewsUrl> mailnewsUrl = do_QueryInterface(m_url, &rv);
      if (NS_FAILED(rv) || !mailnewsUrl) return -1;
      ...
      if (NS_FAILED(rv) || !localFolder) return -1;

      // check if we have a reasonable amount of space left
      rv = localFolder->WarnIfLocalFileTooBig(msgWindow, m_totalDownloadSize,
                                              &spaceNotAvailable);
  ...
  // m_pop3ConData-uidlinfo->hash.  By default, we retrieve messages, unless
  // they have a status, or are too big, in which case we figure out what to do.
  ...
  if (m_pop3ConData->msg_info) {
    Pop3MsgInfo* info =
        m_pop3ConData->msg_info + m_pop3ConData->last_accessed_msg;
    if (m_pop3ConData->only_uidl) {
      ...
      if (info->uidl) {
        Pop3UidlEntry* uidlEntry = (Pop3UidlEntry*)PL_HashTableLookup(
            m_pop3ConData->uidlinfo->hash, info->uidl);
        if (uidlEntry) {
          c = uidlEntry->status;
          popstateTimestamp = uidlEntry->dateReceived;
        }
      ...
      if (c == DELETE_CHAR) {
        ...
        put_hash(m_pop3ConData->newuidl, info->uidl, TOO_BIG,
                 popstateTimestamp);
      } else if (c == TOO_BIG) {
      ...
      if (m_pop3ConData->next_state != POP3_SEND_DELE && info->uidl) {
...
int32_t nsPop3Protocol::XsenderResponse() {
  ...
  if (m_pop3ConData->command_succeeded) {
    if (m_commandResponse.Length() > 4) m_senderInfo = m_commandResponse;
  } else {
 ...
 */
int32_t nsPop3Protocol::SendRetr() {
  char* cmd = PR_smprintf(
      "RETR %ld" CRLF,
      m_pop3ConData->msg_info[m_pop3ConData->last_accessed_msg].msgnum);
  int32_t status = -1;
...
int32_t nsPop3Protocol::RetrResponse(nsIInputStream* inputStream,
  ...
  if (m_pop3ConData->cur_msg_size == -1) {
     ...
     */
    if (!m_pop3ConData->command_succeeded) return Error("pop3RetrFailure");

    /* a successful RETR response looks like: #num_bytes Junk
       from TOP we only get the +OK and data
       */
    if (m_pop3ConData->truncating_cur_msg) { /* TOP, truncated message */
      ...
      if (num) m_pop3ConData->cur_msg_size = atol(num);
      m_commandResponse = newStr;
    }

    /* RETR complete message */
    if (!m_senderInfo.IsEmpty()) flags |= nsMsgMessageFlags::SenderAuthed;
  ...
  {
    // buffer the line we just read in, and buffer all remaining lines in the
    // stream
    status = buffer_size;
    do {
      if (m_pop3ConData->msg_closure) {
  ...
  buffer_size =
      status;  // status holds # bytes we've actually buffered so far...

  /* normal read. Yay! */
  if ((int32_t)(m_bytesInMsgReceived + buffer_size) >
      m_pop3ConData->cur_msg_size)
  ...
  /* meaning _handle_line read ".\r\n" at end-of-msg */
  {
    ...
    if (m_pop3ConData->truncating_cur_msg || m_pop3ConData->leave_on_server) {
      ...
      if ((m_pop3ConData->newuidl) && (info->uidl))
        uidlEntry = (Pop3UidlEntry*)PL_HashTableLookup(m_pop3ConData->newuidl,
                                                       info->uidl);
      ...
      if (uidlEntry && uidlEntry->status == FETCH_BODY &&
        ...
        put_hash(m_pop3ConData->uidlinfo->hash, info->uidl, FETCH_BODY,
                 uidlEntry->dateReceived);
      ...
      if (m_pop3ConData->only_uidl) {
        ...
        uidlEntry = (Pop3UidlEntry*)PL_HashTableLookup(
            m_pop3ConData->uidlinfo->hash, m_pop3ConData->only_uidl);
        NS_ASSERTION(uidlEntry, "uidl not found in uidlinfo");
        if (uidlEntry)
          put_hash(m_pop3ConData->uidlinfo->hash, m_pop3ConData->only_uidl,
                   KEEP, uidlEntry->dateReceived);
      }
    } else {
      m_pop3ConData->next_state = POP3_SEND_DELE;
    }
    ...
    if (m_bytesInMsgReceived < m_pop3ConData->cur_msg_size)
      m_totalBytesReceived +=
          (m_pop3ConData->cur_msg_size - m_bytesInMsgReceived);
  }
...
int32_t nsPop3Protocol::TopResponse(nsIInputStream* inputStream,
                                    uint32_t length) {
  ...
  if (m_pop3ConData->cur_msg_size ==
          -1 &&                          /* first line after TOP command sent */
...
nsresult nsPop3Protocol::HandleLine(char* line, uint32_t line_length) {
  nsresult rv = NS_OK;

  NS_ASSERTION(
      m_pop3ConData->msg_closure,
      "m_pop3ConData->msg_closure is null in nsPop3Protocol::HandleLine()");
  if (!m_pop3ConData->msg_closure) return NS_ERROR_NULL_POINTER;
...
int32_t nsPop3Protocol::DeleResponse() {
   ...
   */
  if (!m_pop3ConData->command_succeeded) return Error("pop3DeleFailure");
      ...
      if (m_pop3ConData->newuidl)
        if (m_pop3ConData->leave_on_server) {
          PL_HashTableRemove(
              m_pop3ConData->newuidl,
              (void*)m_pop3ConData
                  ->msg_info[m_pop3ConData->last_accessed_msg - 1]
                  .uidl);
        } else {
          put_hash(m_pop3ConData->newuidl,
                   m_pop3ConData->msg_info[m_pop3ConData->last_accessed_msg - 1]
                       .uidl,
                   DELETE_CHAR, 0);
        ...
        PL_HashTableRemove(
            host->hash,
            (void*)m_pop3ConData->msg_info[m_pop3ConData->last_accessed_msg - 1]
                .uidl);
    }
  }
...
int32_t nsPop3Protocol::CommitState(bool remove_last_entry) {
      ...
      */
      if (remove_last_entry && m_pop3ConData->msg_info &&
        ...
        Pop3MsgInfo* info =
            m_pop3ConData->msg_info + m_pop3ConData->last_accessed_msg;
        if (info && info->uidl) {
          mozilla::DebugOnly<bool> val =
              PL_HashTableRemove(m_pop3ConData->newuidl, info->uidl);
      ...
      PL_HashTableEnumerateEntries(m_pop3ConData->newuidl,
                                   net_pop3_copy_hash_entries,
                                   (void*)m_pop3ConData->uidlinfo->hash);
    }
  }
...
nsresult nsPop3Protocol::ProcessProtocolState(nsIURI* url,
  ...
  MOZ_LOG(POP3LOGMODULE, LogLevel::Info,
          (POP3LOG("Entering NET_ProcessPop3 %d"), aLength));
  ...
  while (!m_pop3ConData->pause_for_read) {
    ...
    switch (m_pop3ConData->next_state) {
      ...
      case POP3_SEND_PASSWORD:
        if (NS_FAILED(
                StartGetAsyncPassword(POP3_OBTAIN_PASSWORD_BEFORE_PASSWORD)))
          status = -1;
        break;
      ...
      case POP3_QUIT_RESPONSE:
        if (m_pop3ConData->command_succeeded) {
          /*  the QUIT succeeded.  We can now flush the state in popstate.dat
            which keeps track of any uncommitted DELE's */
        ...
        {  // this brace is to avoid compiler error about vars in switch case.
          ...
          if (TestFlag(POP3_PASSWORD_FAILED) && msgWindow) {
            ...
            if (!m_socketIsOpen && mailnewsurl) {
              ...
              // to re-run the url.
              MOZ_LOG(POP3LOGMODULE, LogLevel::Debug,
                      (POP3LOG("need to rerun url because connection dropped "
                               "during auth")));
              m_needToRerunUrl = true;
              return NS_OK;
            }
      ...
      case POP3_FREE: {
        ...
        if (server) {
          MOZ_LOG(POP3LOGMODULE, LogLevel::Debug,
                  (POP3LOG("Clearing server busy in POP3_FREE")));
          server->SetServerBusy(false);  // the server is now not busy
        }
        MOZ_LOG(POP3LOGMODULE, LogLevel::Debug,
                (POP3LOG("Clearing running protocol in POP3_FREE")));
        CloseSocket();
        m_pop3Server->SetRunningProtocol(nullptr);
      ...
      default:
        NS_ERROR(
            "Got to unexpected state in nsPop3Protocol::ProcessProtocolState");
        status = -1;
    } /* end switch */
...
NS_IMETHODIMP nsPop3Protocol::MarkMessages(
    nsTArray<Pop3UidlEntry*>* aUIDLArray) {
  ...
  for (uint32_t i = 0; i < count; i++) {
    bool changed;
    if (m_pop3ConData->newuidl)
      MarkMsgInHashTable(m_pop3ConData->newuidl, aUIDLArray->ElementAt(i),
                         &changed);
    if (m_pop3ConData->uidlinfo)
      MarkMsgInHashTable(m_pop3ConData->uidlinfo->hash,
...
NS_IMETHODIMP nsPop3Protocol::CheckMessage(const char* aUidl, bool* aBool) {
  ...
  if (aUidl) {
    if (m_pop3ConData->newuidl)
      uidlEntry =
          (Pop3UidlEntry*)PL_HashTableLookup(m_pop3ConData->newuidl, aUidl);
    else if (m_pop3ConData->uidlinfo)
      uidlEntry = (Pop3UidlEntry*)PL_HashTableLookup(
          m_pop3ConData->uidlinfo->hash, aUidl);
  }
...
nsresult nsPop3Protocol::GetApopTimestamp() {
  ...
  while (true) {
    ...
    nsCString sub(
        Substring(m_commandResponse, startMark, endMark - startMark + 1));
